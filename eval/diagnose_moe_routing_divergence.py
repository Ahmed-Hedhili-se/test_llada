"""
MoE expert-routing divergence check: model_update vs HF, same long GSM8K
prompt as diagnose_layer_divergence.py.

That script found hidden-state cosine similarity between model_update and
...
between model_update's custom Triton fused-MoE kernel and HF's native MoE
...
Usage:
    python eval/diagnose_moe_routing_divergence.py --weight-dir ./weights
"""
...
def build_long_prompt(tok):
    from eval.correctness.run_math_reasoning_code import _load_gsm8k, SYSTEM_PROMPT_GSM8K
...
def main():
    ...
    print("Running model_update (ours) with router + layer hooks...")
