...
decoding path (_generate_block_cached / generate_cached's confidence_threshold
parameter), on top of the CPU-only selection-logic tests in
eval/test_select_transfer_indices.py.
...
Two things checked, each catching a different failure mode:
  ...
  2. confidence_threshold=<value> (a) never calls the model more than
     ...
     reach since softmax probabilities are <= 1.0, so the fallback-every-
     step path runs for the entire block, exercising the safety net under
     worst-case conditions), and 0.9 (a realistic middle value).
...
def test_threshold_path_bounded_and_complete():
    ...
    # qualifies every step, so each block should finish in far fewer than
    # steps_per_block calls) -- not just "within budget" but actually
    # exercising the early-exit path, otherwise this test wouldn't be
    # distinguishing "works" from "silently always takes the full budget."
    with torch.no_grad():
