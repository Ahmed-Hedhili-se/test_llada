...
----------------------------------------------
``src/server.py`` disables request batching whenever ``tp_size > 1`` (four
...
single endpoint in front of them; ``eval/throughput/run_throughput.py
...
Usage::
    ...
    python -m src.router --backends http://localhost:8001,http://localhost:8002 \\
        --port 8000
