...
Compares:
  ...
  2. Optimized     (dminfr/engine/) — Triton fused MoE, block-wise KV cache, topk=8
"""
...
def load_optimized(weight_dir, device):
    """Load the optimized model from dminfr/engine/ with Triton fused MoE."""
    from dminfr.engine.model import LLaDAMoEKV, FULL_CFG, TritonFusedMoEBlock
...
def optimized_generate_no_cache(model, prompt_ids, gen_length, steps, block_length):
    ...
    dminfr/engine/generate.py's generate_dense docstring."""
    from dminfr.engine.generate import generate_dense
...
def main():
    ...
    ap.add_argument("--confidence-threshold", type=float, default=None,
                    help="Opt-in hierarchical threshold-based decoding (see "
                         ...
                         "dInfer's HierarchyDecoder, in dminfr/engine/generate.py) instead of "
    ...
    ap.add_argument("--no-cache", action="store_true",
                    help="Benchmark the optimized model WITHOUT block-wise KV caching -- "
                         "same dminfr.engine model class/weights (Triton fused MoE, static "
                         "top-8) as the normal optimized path, but every step recomputes the "
                         "full sequence from scratch (dminfr/engine/generate.py's "
    ...
    if is_master:
        print("=" * 80)
        print("  Benchmark: Baseline (src/) vs Optimized (dminfr/engine/)")
        print("=" * 80)
    ...
    if args.mode in ["both", "optimized"]:
        if is_master:
            print("=" * 60)
            if args.no_cache:
                print("  2. OPTIMIZED  (dminfr/engine/, fused MoE, topk=8, NO KV cache)")
            else:
                print("  2. OPTIMIZED  (dminfr/engine/, fused MoE, topk=8, KV cache)")
            print("=" * 60)
    ...
    if args.mode in ["both", "optimized"]:
        ...
        print(f"| {'Optimized (dminfr/engine/, topk=8, KV cache)':<50} | {opt_mean:>10.2f} | {opt_tps:>10.2f} | {speed_text:>10} |")
    print("=" * 100)
