...
Extends benchmarks/correctness/run_correctness.py's MMLU/MMLU-Pro/ARC harness with
...
count for gsm8k (steps is a dminfr/engine/-specific KV-cache parameter, not
...
Usage:
    python benchmarks/correctness/run_math_reasoning_code.py --task gsm8k    --limit 400 --output results/gsm8k.json
    python benchmarks/correctness/run_math_reasoning_code.py --task bbh      --limit 400 --output results/bbh.json
    python benchmarks/correctness/run_math_reasoning_code.py --task cruxeval --limit 400 --output results/cruxeval.json
"""
...
# a diffusion step count (that's a dminfr/engine/-specific KV-cache
...
def extract_final_answer(text: str) -> Optional[str]:
    ...
    dminfr/serving/server.py), and the real answer is always the first occurrence,
...
def main():
    ...
    ap.add_argument("--confidence-threshold", type=float, default=None,
                    help="Opt-in hierarchical threshold-based decoding (see "
                         "dminfr/engine/generate.py's generate_cached, ported from dInfer's "
    ...
    ap.add_argument(
        ...
        help="Use dminfr.engine's generate_dense instead of generate_cached -- "
