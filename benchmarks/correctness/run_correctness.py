...
Usage:
    ...
    python -m benchmarks.correctness.run_correctness \\
        --output results/baseline_mmlu.json

    # Full MMLU
    python -m benchmarks.correctness.run_correctness --limit 0 \\
        --output results/baseline_mmlu_full.json

    # ARC-Challenge
    python -m benchmarks.correctness.run_correctness --task arc_challenge \\
...
# arc_challenge was tried at the paper's 1024/64 generative-benchmark protocol
# (Section 4) and reverted: real evaluation showed it made accuracy WORSE
# (88.0%->60.0% baseline, 80.0%->44.0% with confidence_threshold), not more
# comparable to the paper -- see README's "Adaptive Decoding" section. Uses
# the short DEFAULT_COT_* config below instead, same as every other task.
...
def load_dataset_for_task(task: str, limit: int) -> list[dict]:
    if task in _CODE_TASKS:
        print(
            ...
            f"Use: python benchmarks/correctness/diagnose_humaneval.py\n"
        )
        sys.exit(1)
